    ...
    try:
        ...
        for x in range(1, length+1):
            returnObject.append(
                {"option":
                 response.xpath(
                     xpathTemplate["what_will_learn"][0]+"["+str(x)+"]"+xpathTemplate["what_will_learn"][1]).get()[0:254] or ""
                 }
            )
        ...
        returnObject.append(
            {
                ...
                "designation": response.xpath(xpathTemplate["Instructors"][1]+"["+str(x)+"]"+xpathTemplate["Instructors"][3]).get(),
...
def generate_id(response, xpathTemplate):
    # add Program short code and batch No.
    href = response.xpath(xpathTemplate["generate_id"]).get()
    match = re.search(r'\((?P<code>[A-Z]\w+)\)', href)
    if match:
        return 'HG_' + match.group('code')
    else:
        return 'HG_' + str(href)
...
def get_price_from_string(price_string, response):
    try:
        parsedprice = ''
        if(price_string is None):
            price_string = response.xpath("//div[@class='table-wrapper']/table[@class='col-2-table has_total']/tbody/tr[7]/td[2]/strong/text()").get(
            ) or response.xpath("//div[@class='table-wrapper']/table[@class='col-2-table has_total']/tbody/tr[5]/td[2]/strong/text()").get()
...
def get_all_course_urls(response):
    ...
    for x in range(1, no_of_urls + 1):
        link = str(response.xpath(xpathTemplate["get_all_course_urls"][0] +
                                  "["+str(x)+"]"+xpathTemplate["get_all_course_urls"][1]).get())
        if (link[0] != '/'):
            link = '/' + link
        url = "https://www.hugheseducation.com" + link
        urls.append(url)
    return urls
...
def get_text_date(text = ''):
    text = text.replace('Class Starts','')
    d = dateutil.parser.parse(text)
    return d.isoformat()

class HughesSpider(scrapy.Spider):
    name = 'Hughes'
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(HughesSpider, self).__init__(*args, **kwargs)

    def parse(self, response):
        # print(response.request.url)

        # choose as per the type
        f = open('defaults/hughes.json', "r")
        op_object = json.loads(f.read())
        f.close()

        if response.request.url != "https://www.hugheseducation.com/BrowseCourses":
            xpathTemplate = xpathTemplateCommon['default']

            op_object_new = {
                "id": generate_id(response, xpathTemplate),
                "title": response.xpath(xpathTemplate["title"]).get(),
                "subtitle": response.xpath(xpathTemplate["title"]).get()[0:250],
                ...
                "detail_information": {
                    "duration": {
                        ...
                        "total_duration_in_months": int(float(response.xpath(xpathTemplate["total_duration_in_months"]).get().replace(" Months", ""))),
                ...
                "pricing": {
                    ...
                    "pricing_type": "Paid",
                    "currency": "INR",
                    "regular_price": get_price_from_string(response.xpath(xpathTemplate["regular_price"]).get(), response),
                    "sale_price": get_price_from_string(response.xpath(xpathTemplate["sale_price"]).get(), response),
                ...
                "skills_gained": None,  # array of strings
            ...
            push_to_strapiCMS(op_object, "hughes")
